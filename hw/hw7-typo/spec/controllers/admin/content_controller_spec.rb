...
describe Admin::ContentController do
  ...
  describe 'with admin connection' do
    ...
    describe 'edit action' do
      ...
      it "should merge with other article if the author is the same and delete other be merged article" do
  ...
  describe "merge action" do
    describe "merge succes" do
      ...
      it "should merge two article when user is a admin" do
        comments = [{:author => "other_author1", :body => "comment0"}, 
          {:author => "other_author1", :body => "comment1"}, 
          {:author => "other_author2", :body => "comment2"}]
        article1 = Article.create(:title => "Test merge1", :author => "test_author", :body => "test1page")
        article1.comments.create(comments[0])
        article2 = Article.create(:title => "Test merge2", :author => "test_author", :body => "test2page")
        article2.comments.create(comments[1])
        article2.comments.create(comments[2])
        article1.save!
        article2.save!
        ...
        comment1_str = article1.comments.map do |comment|
          "#{comment.author} #{comment.body}"
        end
        comment2_str = comments.map do |comment|
          "#{comment[:author]} #{comment[:body]}"
        end
        expect(comment1_str).to eq comment2_str
        expect { Article.find(article2.id) }.to raise_error(ActiveRecord::RecordNotFound)
        expect(article1.body).to eq "test1page\n" + "test2page"
      end
    end

    describe "merge fail"  do
      before do
        Factory(:blog)
      ...
      it "should not merge two article when user is not a admin" do
        post :merge, :id => "1", :merge_with => "2"
        expect(response).to redirect_to(admin_content_path)
        expect(flash[:error]).to eq _("Error, you are not allowed to perform this action")
      end
    end
  end
